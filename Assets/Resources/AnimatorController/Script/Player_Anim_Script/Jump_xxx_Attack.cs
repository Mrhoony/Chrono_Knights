...
public class Jump_xxx_Attack : AnimatorManager
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        if (stateInfo.normalizedTime > 0.2f)
        {
            ...
            {
                move = true;
                playerControl.Attack(AtkType.spear_Jump_XXX_Attack, playerControl.AttackDistance(AtkType.spear_Jump_XXX_Attack));
            }
        }
    }
    ...
    {
        if (!animator.GetBool("isJump_y_attack") && !animator.GetBool("isJump_up_x_attack") && !animator.GetBool("isJump_down_x_attack"))
        {
