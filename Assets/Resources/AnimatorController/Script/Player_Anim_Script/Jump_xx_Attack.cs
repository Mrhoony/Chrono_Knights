...
public class Jump_xx_Attack : AnimatorManager
{
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        if (stateInfo.normalizedTime > 0.2f)
        {
            ...
            {
                move = true;
                playerControl.Attack(AtkType.spear_Jump_XX_Attack, playerControl.AttackDistance(AtkType.spear_Jump_XX_Attack));
            }
        }
    }
    ...
    {
        if (!animator.GetBool("isJump_xxx_attack") && !animator.GetBool("isJump_y_attack") && !animator.GetBool("isJump_up_x_attack") && !animator.GetBool("isJump_down_x_attack"))
        {
