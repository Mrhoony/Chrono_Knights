...
public class ProjectileObjectArrow : MonoBehaviour
{
    ...
    public void arrowShooting(int _Damage, float _DistanceX, float _DistanceY, int _ArrowDirection)
    {
        ...
        if(_DistanceY != 0)
        {
            rb.AddForce(new Vector2(_ArrowDirection * _DistanceX * 2f, 1/ _DistanceY), ForceMode2D.Impulse);
        }
        else
        {
            rb.AddForce(new Vector2(_ArrowDirection * _DistanceX * 2f, 4f), ForceMode2D.Impulse);
        }
    }
